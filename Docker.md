...
#### 5.2 方案一：CentOS / 云服务器安装 Docker（yum）
...
#### 5.3 方案二：Ubuntu / VirtualBox 虚拟机安装 Docker（apt）
...
#### 5.4 方案三：WSL 2 + Docker Desktop（Windows GUI）
...
> **注意**：修改已挂载的静态文件通常会立即反映到容器中；如果浏览器没变化，优先强制刷新或清缓存。修改 Nginx 配置文件后，需要执行 `nginx -s reload` 或重启容器才会生效。修改挂载关系本身（例如新增一条 `-v`）则需要重新创建容器。
...
# 5. 修改静态文件后，浏览器刷新即可；修改 nginx 配置后再重载或重启
...
#### 10.6 Docker Desktop 部署 Nginx 实战

> 适用场景：Windows 上使用 Docker Desktop，本地启动一个 Nginx，用来访问静态网页，或者作为后端服务的统一入口。
...
| 参数 | 含义 |
|------|------|
| `nginx:alpine` | 使用轻量版 Nginx 镜像 |
| `--name nginx_demo` | 容器名称 |
...
```powershell
mkdir html
Set-Content .\html\index.html "<h1>Hello Nginx from Docker Desktop</h1>"
```
...
##### 3. 为什么要把本地配置文件挂载到 Nginx 容器

容器应该是可以删除、可以重建、可以复现的。不要把重要配置只手动改在容器内部。

```text
本地文件 ./nginx/default.conf
  ↓ 挂载
容器文件 /etc/nginx/conf.d/default.conf
```

这样做的好处：

- 配置文件跟着项目走，别人拿到项目后可以直接 `docker compose up -d`
- 以后只需要修改本地配置文件，不需要进入容器手动编辑
- 容器删除重建后，配置不会丢失
- 配置文件可以被 Git 记录，方便追踪修改历史
...
```powershell
docker exec nginx_demo nginx -t
```

配置正确后重载：

```powershell
docker exec nginx_demo nginx -s reload
```
...
##### 5. 常见错误：YAML 不能使用 Tab 缩进

`docker-compose.yml` 只能使用空格缩进，不能使用 Tab。
...
如果出现下面错误，优先检查缩进里有没有 Tab：

```text
yaml: while scanning for the next token at line 2: found character that cannot start any token
```



#### 10.7 Nginx 和后端服务都放到 Compose 中

如果 Nginx 和后端服务都由同一个 `docker-compose.yml` 启动，它们会自动加入同一个 Compose 网络。容器之间访问时，不写 `localhost`，而是写 **服务名**。
...
| 写法 | 转发效果 |
|------|----------|
| `proxy_pass http://backend:3000/;` | `/api/users` 转成后端的 `/users` |
| `proxy_pass http://backend:3000;` | `/api/users` 原样转给后端 `/api/users` |

如果后端没有放进 Compose，而是直接跑在 Windows 宿主机上，例如 `http://localhost:3000`，Nginx 容器里不要写 `localhost:3000`，应该写：

```nginx
proxy_pass http://host.docker.internal:3000;
```

因为容器里的 `localhost` 指的是容器自己，不是 Windows 电脑。



#### 10.8 内网穿透 + Nginx + 后端服务访问链路

本地项目需要临时给别人访问时，可以使用内网穿透工具把公网地址转发到自己电脑端口，再由 Nginx 转发到后端。
...
内网穿透服务
  ↓
你的电脑某个端口，例如 localhost:8080
  ↓
Docker 端口映射到 Nginx 容器 80
  ↓
Nginx 反向代理
  ↓
后端服务
...
```text
公网穿透地址 -> localhost:8080
Docker: localhost:8080 -> nginx:80
Nginx: /api -> backend:3000
```

这种方式适合临时演示、测试第三方回调、给朋友预览项目。正式长期上线更建议部署到云服务器，并配置 HTTPS、防火墙、安全组、日志和监控。
